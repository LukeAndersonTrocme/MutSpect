...
Remove0.001<-join[which(join$NAG > 0.001 & join$JPT == 0| join$JPT > 0.001 & join$NAG == 0),]
...
write.table(Remove0.001[,c(1,2)], file=paste(out,"RemoveSites_0.001.txt",sep=""), quote=F, row.names=F, col.names=F, sep = "\t")
