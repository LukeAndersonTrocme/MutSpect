...
pops=['JPT','CHB','CHS','CDX','KHV', 'NAG']
for pop in pops:
    #pop_counts[pop]=frequency_breakdown(pop,22)-frequency_breakdown_repeats(pop,22)-frequency_breakdown_phyloP(pop,22)
    pop_counts[pop]=frequency_breakdown(pop,22)
    num_variants[pop]=pop_counts[pop].sum()

name=dict({})
for p in zip(pops,['Japanese','Northern Han','Southern Han','Dai','Kinh', 'New_NAG']):
#for p in zip(pops,['Japanese','Northern Han','Southern Han','Dai','Kinh']):
    name[p[0]]=p[1]

for pop_ind1 in range(len(pops)):
    refpop=pops[pop_ind1]
    subplot_ind=1
    for pop_ind2 in range(pop_ind1)+range(pop_ind1+1,len(pops)):
        ratio_list=[]
        pop=pops[pop_ind2]
        print refpop, pop
        ...
        for i in range(row):
            for j in range(col):
                ...
                if this_pval<0.00001:
                    sig_x.append(j+0.5)
                    sig_y.append(i+0.5)
        plt.subplot(1,len(pops),subplot_ind)
        subplot_ind+=1
...
#    plt.clf()
    ...
    fig=plt.gcf()
    plt.savefig(args.outputroot+'ASN_heatmap_v_'+refpop+'_nosingle.pdf',format='pdf')
    plt.clf()
